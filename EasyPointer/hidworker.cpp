...
void CHidWorker::run()
{
    while (!m_bEndWork)
    {
        ...
        while(true)
        {
            ...
            memset(szBuf,0,64);
            szBuf[0] = 0x0C ;
            int nRet = hid_read_timeout(pDev,szBuf,64,50000);
            //hid_send_feature_report(pDev,szBuf,65);
            QThread::sleep(20);
            //int nRet = hid_get_feature_report(pDev,szBuf,65);
            //int nRet = hid_get_input_report(pDev,szBuf,64);
            if(nRet == -1)
            {
